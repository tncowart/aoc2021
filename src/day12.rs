...
fn visit_caves<'a>(
    ...
    if node == "end" {
        ...
        (&caves[node] - visited).iter().fold(Vec::new(), |mut acc, n| {
...
pub fn day12() {
    let caves = include_str!("../resources/day12.txt")
        .lines().filter(|l| !(*l).is_empty()).map(|l| l.split_once('-').unwrap())
        .fold(
    ...
    println!("Day 12.1: {}", visit_caves("start", &caves, &HashSet::from(["start"]), &Vec::new(), None).len());

    println!("Day 12.2: {}", caves.keys().filter(|k| is_lower(k) && **k != "end" && **k != "start")
            .fold(HashSet::new(), |mut acc, k| {
                acc.extend(visit_caves("start", &caves, &HashSet::from(["start"]), &Vec::new(), Some(k)));
                acc
            }).len()
    );
}
